...
def playerHasHitBaddie(playerRect, badhit):
    for b in badhit:
...
pygame.display.set_caption('Wrong side Rage')
pygame.mouse.set_visible(False)


pygame.mixer.music.load('crrr.wav')
...
while (count > 0):
    ...
    while True:
        ...
        if baddieAddCounter == ADDNEWBADDIERATE:
            baddieAddCounter = 0
            badhitize = 30
            newBaddie = {'rect': pygame.Rect(random.randint(140, 485), 0 - badhitize, 23, 47),
                         'speed': random.randint(BADDIEMINSPEED, BADDIEMAXSPEED),
                         'surface': pygame.transform.scale(random.choice(sample), (23, 47)),
                         }
            badhit.append(newBaddie)
            sideLeft = {'rect': pygame.Rect(0, 0, 126, 600),
                        'speed': random.randint(BADDIEMINSPEED, BADDIEMAXSPEED),
                        'surface': pygame.transform.scale(wallLeft, (126, 599)),
                        }
            badhit.append(sideLeft)
            sideRight = {'rect': pygame.Rect(497, 0, 303, 600),
                         'speed': random.randint(BADDIEMINSPEED, BADDIEMAXSPEED),
                         'surface': pygame.transform.scale(wallRight, (303, 599)),
                         }
            badhit.append(sideRight)
        ...
        if playerHasHitBaddie(playerRect, badhit):
            if score > topScore:
                topScore = score
            break
